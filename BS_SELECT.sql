...
-- 4. EMPLOYEE 테이블의 사원명, 이메일, 전화번호, 고용일 조회

SELECT EMP_NAME AS 사원명, EMAIL AS 이메일, PHONE AS 전화번호, HIRE_DATE AS 고용일
FROM EMPLOYEE;

-- 5. EMPLOYEE 테이블의 고용일, 사원명, 월급 조회

SELECT HIRE_DATE AS 고용일, EMP_NAME AS 사원명, SALARY AS 월급
FROM EMPLOYEE;

-- 6.  EMPLOYEE 테이블에서 사원명, 연봉, 총수령액(보너스포함), 실수령액(총수령액 - (연봉*세금 3%)) 조회

SELECT EMP_NAME AS 사원명,
        SALARY * 12 AS 연봉,
        ((SALARY * 12) + ((SALARY * 12) * NVL(BONUS, 0))) AS 총수령액,
        ((SALARY * 12) + ((SALARY * 12) * NVL(BONUS, 0))) - ABS((SALARY * 12) * 0.03) AS 실수령액
FROM EMPLOYEE;

-- 7. EMPLOYEE 테이블에서 SAL_LEVEL이 S1인 사원명, 월급, 고용일, 전화번호 조회

SELECT EMP_NAME AS 사원명, SALARY AS 월급, HIRE_DATE AS 고용일, PHONE AS 전화번호
FROM EMPLOYEE
WHERE SAL_LEVEL = 'S1';

-- 8. EMPLOYEE 테이블에서 실수령액(6번 참고)이 5천만원 이상인 사원명, 월급, 실수령액, 고용일 조회
...
-- 9. EMPLOYEE 테이블에 월급이 4,000,000원이상 이고 JOB_CODE가 J2인 사원의 전체 내용 조회

SELECT *
FROM EMPLOYEE
WHERE SALARY >= 4000000 AND JOB_CODE = 'J2';

-- 10. EMPLOYEE 테이블에 DEPT_CODE가 D9이거나 D5인 사원 중 고용일이 02년 1월 1일보다 빠른 사원의 사원명, 부서코드, 고용일 조회

SELECT EMP_NAME AS 사원명, DEPT_CODE AS 부서코드, HIRE_DATE AS 고용일
FROM EMPLOYEE
...
SELECT EMP_NAME AS 사원명, REPLACE(EMP_NO, SUBSTR(EMP_NO, 8), '*******') AS 주민번호
FROM EMPLOYEE;

-- 17. EMPLOYEE 테이블에서 사원명, 입사일-오늘, 오늘-입사일 조회
-- *년도에 따라 일수가 다를 수 있음, 참고 : (단, 각 별칭은 근무일수1, 근무일수2가 되도록 하고 모두 정수(내림), 양수가 되도록 처리)

SELECT EMP_NAME AS 사원명, ABS(FLOOR(HIRE_DATE - SYSDATE)) AS "근무일수(입사일 - 오늘)", FLOOR(SYSDATE - HIRE_DATE) AS "근무일수(오늘 - 입사일)" 
FROM EMPLOYEE;

-- 18. EMPLOYEE 테이블에서 사원번호가 홀수인 직원들의 정보 모두 조회

SELECT *
FROM EMPLOYEE
WHERE MOD(EMP_ID, 2) != 0;

-- 19.  EMPLOYEE 테이블에서 근무 년수가 25년 이상인 직원 정보 조회
...
SELECT EMP_NAME AS 사원명, TO_CHAR(SALARY, 'L999,999,999') AS 급여
FROM EMPLOYEE;

-- 21.  EMPLOYEE 테이블에서 사원명, 부서코드, 생년월일, 나이(만) 조회
...
SELECT EMP_ID AS 직원아이디, EMP_NAME AS 이름, DEPT_CODE AS 부서코드, 
           CASE
