...
SELECT EMPNO AS 사원번호, ENAME AS 사원명, DEPTNO AS 직급, HIREDATE AS 입사일
FROM EMP
ORDER BY HIREDATE;
...
SELECT EMPNO AS 사원번호, ENAME AS 사원명, SUBSTR(SAL, -3, 3) AS 급여
FROM EMP
ORDER BY SAL DESC;
...
SELECT ENAME AS 사원명, EXTRACT(YEAR FROM HIREDATE) AS 입사년도, EXTRACT(MONTH FROM HIREDATE) AS 입사월 
FROM EMP;
...
SELECT EMPNO AS 사원번호, ENAME AS 사원명, HIREDATE AS 입사일, TO_DATE(ADD_MONTHS(HIREDATE, 280), 'RR/MM/DD') AS "40년 후..."
FROM EMP;
